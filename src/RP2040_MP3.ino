...
namespace Config {
    ...
    const int SD_CS = 5;
    const int BTN_PLAY = 6, BTN_NEXT = 7, BTN_BACK = 15;
...
void updateProgressBar() {
    ...
    uint32_t currentPos = source ? source->getPos() : 0;

    if (abs((int)(currentPos - lastPos)) < 10000) return;
    lastPos = currentPos;

    int progressWidth = map(currentPos, 0, fileSize, 0, 200);
...
void renderMainUI() {
    ...
    int volBar = map(volume, 0, 100, 0, 200);
...
void loadTrack(int index) {
    ...
    source = new AudioFileSourceSD(fileList[index].c_str());
    if (source) {
...
void setup() {
    ...
    while (File entry = root.openNextFile()) {
        ...
        n.toLowerCase();
        if (!entry.isDirectory() && n.endsWith(".mp3") && fileCount < Config::MAX_FILES) {
    ...
    out->SetPinout(18, 19, 20); // BCLK, LRCK, DIN
...
void loop() {
    ...
    if (millis() - lastBtn > 250) {
        if (!digitalRead(Config::BTN_PLAY)) {
            isPlaying = !isPlaying;
            isPlaying ? mp3->begin(source, out) : mp3->stop();
            renderMainUI();
            lastBtn = millis();
        }
    ...
    if (currentClk != lastClk) {
        if (digitalRead(Config::ENC_DT) != currentClk) {
            isMenuOpen ? selectedTrack++ : volume += 5;
        } else {
            isMenuOpen ? selectedTrack-- : volume -= 5;
        }
        ...
        if (!isMenuOpen) out->SetGain(volume / 100.0f);
